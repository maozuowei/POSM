...
//	public final static int EXPIRE_MONTH = 10;
	public final static int EXPIRE_WEEK = 60 * 60 * 24 * 7;
	...
	static {
		...
		// config.setMaxTotal(CardPayConstant.REDIS_POOL_MAX_TOTAL);
		config.setMaxTotal(60000);
		// config.setMaxIdle(CardPayConstant.REDIS_POOL_MAX_IDLE);
		config.setMaxIdle(300);
		config.setMaxWaitMillis(2000);
		config.setTestWhileIdle(true);
		config.setTestOnReturn(false);
		config.setTestOnBorrow(true);
		// config.setMinIdle(CardPayConstant.REDIS_POOL_MIN_IDLE);
		config.setMinIdle(5);
//		 jedisPool = new JedisPool(config, "172.16.1.116", 6379, 10000,
//		 "123456",1);
		log.info("---------------初始化jedisPool池----START---------");
		jedisPool = new JedisPool(config, DataDictInitialize.REDIS_HOST, DataDictInitialize.REDIS_PORT,
				DataDictInitialize.REDIS_TIME_OUT, DataDictInitialize.REDIS_PASSWORD,DataDictInitialize.REDIS_DATABASE);
		log.info("---------------初始化jedisPool池----END---------");
	}
	...
	public class Objects {
		...
		public Object getObject(String key) {
			...
			try {
				sjedis = getJedis();
				if (sjedis.exists(getBytesKey(key))) {
					value = ObjectUtils.unserialize(sjedis.get(getBytesKey(key)));
				}
			} catch (Exception e) {
				log.error("getObject " + key + " = " + value + " 异常:" + e);
			} finally {
				returnJedis(sjedis);
			}
			return value;
		...
		public String setObject(String key, Object value, int cacheSeconds) {
			...
			try {
				...
				result = sjedis.set(getBytesKey(key), ObjectUtils.serialize(value));
				if (cacheSeconds != 0) {
					sjedis.expire(key, cacheSeconds);
				}
			} catch (Exception e) {
				log.error("setObject " + key + " = " + value + " 异常:" + e);
			} finally {
				returnJedis(sjedis);
			}
			return result;
...
//		String param = "15024461119" + "8644260395002530000000000000000000000000" + loginTime;
//		String newToken = MD5.md5(param).toUpperCase();
//		LoginInfoObject lios = new LoginInfoObject(newToken, "8644260395002530000000000000000000000000",
//				System.currentTimeMillis(), "000027");
//		System.out.println("lios:" + lios);
//		System.out.println("token:" + newToken);
//
		JedisUtil jedisUtil = JedisUtil.getInstance();
		JedisUtil.Objects objects = jedisUtil.new Objects();
//		String setObject = objects.setObject(newToken, lios, EXPIRE_MONTH);
//		System.out.println(setObject);
//		if (objects.exists(newToken)) {
			LoginInfoObject lios2 = (LoginInfoObject) objects.getObject("");
//			System.out.println("logNo:" + lios2.getLogNo() + "  serialNum:" + lios2.getMobileSerialNum()
//					+ "  loginTime:" + lios2.getLoginTime() + " token:" + lios2.getToken());
//		} else {
