 ...
 *
 */
public class GetPublicUtilitiesItems extends ClientReqHandle implements
	...
	public CommunicationInfo getCommuInfo(HashMap<String, String> map) {
		...
		webserviceInput.put("TRADE_CODE", "GJ0004");// 公共事业缴费项目列表
	...
	public HashMap<String, String> handleReturnMessage(
		...
		resultJson.put("resultBean", jsonObj.getString("TX_RECORD_SET").replaceAll("\\\\", ""));
	...
	public LocalResp verifyMessage(HashMap<String, String> map) {
		...
		if(map.get(ParseCoreReceiveXml.CITY_CODE) == null
				|| map.get(ParseCoreReceiveXml.CITY_CODE).trim()
				.length() == 0){
			i++;
			sb.append(ParseCoreReceiveXml.CITY_CODE+",");
		}
		if (i > 0) {
