...
public class UserUpdateInfo extends ClientReqHandle implements
	...
	public LocalResp verifyMessage(HashMap<String, String> map) {
		...
		if (map.get(ParseCoreReceiveXml.USER_TYPE) == null
				|| map.get(ParseCoreReceiveXml.USER_TYPE).trim()
						.length() == 0) {
			i++;
			sb.append(ParseCoreReceiveXml.USER_TYPE + ",");
		}
		if (i > 0) {
