...
public class BindQrcodeUrl extends ClientReqHandle implements IClientReqHandleService {
	...
	public CommunicationInfo getCommuInfo(HashMap<String, String> map) {
		...
		String parameter = ParseCoreReceiveXml.MOBILE_NO + "=" + map.get(ParseCoreReceiveXml.MOBILE_NO) + "&" + 
				...
				ParseCoreReceiveXml.QRCODEURL + "=" + map.get(ParseCoreReceiveXml.QRCODEURL);
	...
	public LocalResp verifyMessage(HashMap<String, String> map) {
		...
		if (map.get(ParseCoreReceiveXml.QRCODEURL) == null || map.get(ParseCoreReceiveXml.QRCODEURL).trim().length() == 0) {
			i++;
			sb.append(ParseCoreReceiveXml.QRCODEURL + ",");
		}
		if (i > 0) {
