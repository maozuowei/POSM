 ...
 * 
 */
public class TerminalManage extends ClientReqHandle implements IClientReqHandleService {
	...
	public CommunicationInfo getCommuInfo(HashMap<String, String> map) {
		...
		String parameter = ParseCoreReceiveXml.MOBILE_NO + "=" + map.get(ParseCoreReceiveXml.MOBILE_NO) + "&" + 
					ParseCoreReceiveXml.APP_USER + "=" + map.get(ParseCoreReceiveXml.APP_USER) + "&" + 
					ParseCoreReceiveXml.DEVICE_ID + "=" + map.get(ParseCoreReceiveXml.DEVICE_ID) + "&" + 
					ParseCoreReceiveXml.PSAM_ID + "=" + map.get(ParseCoreReceiveXml.PSAM_ID) + "&" + 
					ParseCoreReceiveXml.OPT_TYPE + "=" + map.get(ParseCoreReceiveXml.OPT_TYPE);
	...
	public LocalResp verifyMessage(HashMap<String, String> map) {
		...
		if("2".equals(map.get(ParseCoreReceiveXml.OPT_TYPE))||"3".equals(map.get(ParseCoreReceiveXml.OPT_TYPE))){
			
			if (map.get(ParseCoreReceiveXml.DEVICE_ID) == null || map.get(ParseCoreReceiveXml.DEVICE_ID).trim().length() == 0) {
				i++;
				sb.append(ParseCoreReceiveXml.DEVICE_ID + ",");
			}
		}
		if("2".equals(map.get(ParseCoreReceiveXml.OPT_TYPE))){ 
			if (map.get(ParseCoreReceiveXml.PSAM_ID) == null || map.get(ParseCoreReceiveXml.PSAM_ID).trim().length() == 0) {
				i++;
				sb.append(ParseCoreReceiveXml.PSAM_ID + ",");
			}
		}
