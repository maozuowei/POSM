 ...
 * 
 */
public class UserIdentityPicUpload extends ClientReqHandle implements IClientReqHandleService {
	...
	public CommunicationInfo getCommuInfo(HashMap<String, String> map) {
		...
		try {
			parameter = ParseCoreReceiveXml.APP_USER + "=" + map.get(ParseCoreReceiveXml.APP_USER) + "&" + ParseCoreReceiveXml.MOBILE_NO + "="
					...
					+ ParseCoreReceiveXml.IMG + "=" + URLEncoder.encode(map.get(ParseCoreReceiveXml.IMG), "UTF-8") + "&" + ParseCoreReceiveXml.IMG_APPLY_TYPE + "="
					+ map.get(ParseCoreReceiveXml.IMG_APPLY_TYPE) + "&" + ParseCoreReceiveXml.IMG_SIGN + "=" + map.get(ParseCoreReceiveXml.IMG_SIGN);
