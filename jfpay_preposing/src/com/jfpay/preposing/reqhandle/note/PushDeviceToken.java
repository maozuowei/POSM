 ...
 * 
 */
public class PushDeviceToken extends ClientReqHandle implements
	...
	public CommunicationInfo getCommuInfo(HashMap<String, String> map) {
		...
		String parameter = ParseCoreReceiveXml.MOBILE_NO + "="
				...
				+ ParseCoreReceiveXml.DEVICE_TOKEN + "="
				+ map.get(ParseCoreReceiveXml.DEVICE_TOKEN) + "&"
				+ ParseCoreReceiveXml.CLIENT_TYPE + "="
				+ map.get(ParseCoreReceiveXml.CLIENT_TYPE) + "&"
	...
	public LocalResp verifyMessage(HashMap<String, String> map) {
		...
		if (map.get(ParseCoreReceiveXml.DEVICE_TOKEN) == null
				|| map.get(ParseCoreReceiveXml.DEVICE_TOKEN).trim().length() == 0) {
			i++;
			sb.append(ParseCoreReceiveXml.DEVICE_TOKEN + ",");
		}
		if (i > 0) {
