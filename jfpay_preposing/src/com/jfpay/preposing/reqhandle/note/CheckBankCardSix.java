...
public class CheckBankCardSix extends ClientReqHandle 
	...
	public CommunicationInfo getCommuInfo(HashMap<String, String> map) {
		...
		String parameter = ParseCoreReceiveXml.REAL_NAME + "=" + map.get(ParseCoreReceiveXml.REAL_NAME)+ "&" + 
						...
						ParseCoreReceiveXml.EXP_INFO + "=" + map.get(ParseCoreReceiveXml.EXP_INFO);
	...
	public LocalResp verifyMessage(HashMap<String, String> map) {
		...
		if (map.get(ParseCoreReceiveXml.EXP_INFO) == null || 
				map.get(ParseCoreReceiveXml.EXP_INFO).trim().length() == 0) {
			i++;
