...
public class UserHeadPicUpload extends ClientReqHandle implements IClientReqHandleService {
		...
		// TODO Auto-generated method stub
				...
				try {
					parameter = ParseCoreReceiveXml.APP_USER + "=" + map.get(ParseCoreReceiveXml.APP_USER) + "&" + ParseCoreReceiveXml.MOBILE_NO + "="
							+ map.get(ParseCoreReceiveXml.MOBILE_NO) + "&"+ParseCoreReceiveXml.HEAD_PIC + "=" + URLEncoder.encode(map.get(ParseCoreReceiveXml.HEAD_PIC), "UTF-8");
	...
	public LocalResp verifyMessage(HashMap<String, String> map) {
		...
		if (map.get(ParseCoreReceiveXml.HEAD_PIC)==null||map.get(ParseCoreReceiveXml.HEAD_PIC).trim().length()==0) {
			i++;
			sb.append(ParseCoreReceiveXml.HEAD_PIC+",");
		}
		if (i > 0) {
