 ...
 * 
 */
public class UnbindQuickBankCard extends ClientReqHandle implements
	...
	public CommunicationInfo getCommuInfo(HashMap<String, String> map) {
		...
		String parameter = ParseCoreReceiveXml.MOBILE_NO + "="
				...
				+ ParseCoreReceiveXml.BIND_ID + "="
				+ map.get(ParseCoreReceiveXml.BIND_ID) + "&"
	...
	public LocalResp verifyMessage(HashMap<String, String> map) {
		...
		if (map.get(ParseCoreReceiveXml.BIND_ID) == null
				|| map.get(ParseCoreReceiveXml.BIND_ID).trim().length() == 0) {
			i++;
			sb.append(ParseCoreReceiveXml.BIND_ID + ",");
		}
		if (i > 0) {
