...
public class CouponOrderConfim extends ClientReqHandle implements IClientReqHandleService{
	...
	public CommunicationInfo getCommuInfo(HashMap<String, String> map) {
		...
		parameter=ParseCoreReceiveXml.COUPON_ORDER_ID+"="+map.get(ParseCoreReceiveXml.COUPON_ORDER_ID)+"&"
				 ...
				 +ParseCoreReceiveXml.BUY_PRICE+"="+map.get(ParseCoreReceiveXml.BUY_PRICE)+"&"
				 +ParseCoreReceiveXml.BUY_NUM+"="+map.get(ParseCoreReceiveXml.BUY_NUM)+"&"
				 +ParseCoreReceiveXml.BUY_TOTALMONEY+"="+map.get(ParseCoreReceiveXml.BUY_TOTALMONEY)+"&"
	...
	public LocalResp verifyMessage(HashMap<String, String> map) {
		...
		if (map.get(ParseCoreReceiveXml.BUY_PRICE)==null||map.get(ParseCoreReceiveXml.BUY_PRICE).trim().length()==0) {
			i++;
			sb.append(ParseCoreReceiveXml.BUY_PRICE+",");
		}
		if (map.get(ParseCoreReceiveXml.BUY_NUM)==null||map.get(ParseCoreReceiveXml.BUY_NUM).trim().length()==0) {
			i++;
			sb.append(ParseCoreReceiveXml.BUY_NUM+",");
		}
		if (map.get(ParseCoreReceiveXml.BUY_TOTALMONEY)==null||map.get(ParseCoreReceiveXml.BUY_TOTALMONEY).trim().length()==0) {
			i++;
			sb.append(ParseCoreReceiveXml.BUY_TOTALMONEY+",");
		}
