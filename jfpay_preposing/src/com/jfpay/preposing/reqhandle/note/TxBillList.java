...
public class TxBillList extends ClientReqHandle implements IClientReqHandleService{
	...
	public CommunicationInfo getCommuInfo(HashMap<String, String> map) {
		...
		String parameter = "";
		parameter=ParseCoreReceiveXml.CUSTOMER_ID+"="+map.get(ParseCoreReceiveXml.CUSTOMER_ID)+"&"+ParseCoreReceiveXml.LAST_MSG_ID+"="+map.get(ParseCoreReceiveXml.LAST_MSG_ID)+"&"
				+ParseCoreReceiveXml.MSG_SIZE+"="+map.get(ParseCoreReceiveXml.MSG_SIZE);
	...
	public LocalResp verifyMessage(HashMap<String, String> map) {
		...
		if (map.get(ParseCoreReceiveXml.LAST_MSG_ID)==null||map.get(ParseCoreReceiveXml.LAST_MSG_ID).trim().length()==0) {
			i++;
			sb.append(ParseCoreReceiveXml.LAST_MSG_ID+",");
		}
