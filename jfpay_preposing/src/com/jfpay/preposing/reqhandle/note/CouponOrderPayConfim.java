...
public class CouponOrderPayConfim extends ClientReqHandle implements IClientReqHandleService{
	...
	public CommunicationInfo getCommuInfo(HashMap<String, String> map) {
		...
		parameter=ParseCoreReceiveXml.COUPON_ORDER_ID+"="+map.get(ParseCoreReceiveXml.COUPON_ORDER_ID)+"&"
				 ...
				 +ParseCoreReceiveXml.PAYTYPE+"="+map.get(ParseCoreReceiveXml.PAYTYPE)+"&"
				 +ParseCoreReceiveXml.PAY_ORDERID+"="+map.get(ParseCoreReceiveXml.PAY_ORDERID)+"&"
				 +ParseCoreReceiveXml.PAY_STATUS+"="+map.get(ParseCoreReceiveXml.PAY_STATUS);
	...
	public LocalResp verifyMessage(HashMap<String, String> map) {
		...
		if (map.get(ParseCoreReceiveXml.PAYTYPE)==null||map.get(ParseCoreReceiveXml.PAYTYPE).trim().length()==0) {
			i++;
			sb.append(ParseCoreReceiveXml.PAYTYPE+",");
		}
		if (map.get(ParseCoreReceiveXml.PAY_ORDERID)==null||map.get(ParseCoreReceiveXml.PAY_ORDERID).trim().length()==0) {
			i++;
			sb.append(ParseCoreReceiveXml.PAY_ORDERID+",");
		}
		if (map.get(ParseCoreReceiveXml.PAY_STATUS)==null||map.get(ParseCoreReceiveXml.PAY_STATUS).trim().length()==0) {
			i++;
			sb.append(ParseCoreReceiveXml.PAY_STATUS+",");
		}
		if (i > 0) {
