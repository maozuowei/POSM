...
public class GetUserInstruction extends ClientReqHandle implements IClientReqHandleService {
	...
	public CommunicationInfo getCommuInfo(HashMap<String, String> map) {
		...
		String parameter = ParseCoreReceiveXml.INSTRUCTION_VERSION + "=" + map.get(ParseCoreReceiveXml.INSTRUCTION_VERSION) + "&" + ParseCoreReceiveXml.VERSION + "="
				+ map.get(ParseCoreReceiveXml.VERSION) + "&" + ParseCoreReceiveXml.APP_USER + "=" + map.get(ParseCoreReceiveXml.APP_USER);
	...
	public LocalResp verifyMessage(HashMap<String, String> map) {
		...
		if (map.get(ParseCoreReceiveXml.INSTRUCTION_VERSION) == null || map.get(ParseCoreReceiveXml.INSTRUCTION_VERSION).trim().length() == 0) {
			i++;
