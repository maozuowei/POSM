 ...
 * 
 */
public class RecordDetail extends ClientReqHandle implements IClientReqHandleService {
	...
	public CommunicationInfo getCommuInfo(HashMap<String, String> map) {
		...
		String parameter = ParseCoreReceiveXml.APP_USER + "=" + map.get(ParseCoreReceiveXml.APP_USER) +"&"+ ParseCoreReceiveXml.MOBILE_NO + "=" + map.get(ParseCoreReceiveXml.MOBILE_NO)
					+ "&" + ParseCoreReceiveXml.RECORDID + "=" + map.get(ParseCoreReceiveXml.RECORDID);
	...
	public LocalResp verifyMessage(HashMap<String, String> map) {
		...
		if (map.get(ParseCoreReceiveXml.RECORDID) == null || map.get(ParseCoreReceiveXml.RECORDID).trim().length() == 0) {
			i++;
			sb.append(ParseCoreReceiveXml.RECORDID + ",");
		}
		if (i > 0) {
