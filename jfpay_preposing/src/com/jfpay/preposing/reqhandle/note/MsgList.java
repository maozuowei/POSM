 ...
 * 
 */
public class MsgList extends ClientReqHandle implements IClientReqHandleService {
	...
	public CommunicationInfo getCommuInfo(HashMap<String, String> map) {
		...
		String parameter = ParseCoreReceiveXml.MOBILE_NO + "="
				...
				+ ParseCoreReceiveXml.FIRST_MSG_ID + "="
				+ map.get(ParseCoreReceiveXml.FIRST_MSG_ID) + "&"
				+ ParseCoreReceiveXml.LAST_MSG_ID + "="
				+ map.get(ParseCoreReceiveXml.LAST_MSG_ID) + "&"
				+ ParseCoreReceiveXml.MSG_SIZE + "="
				+ map.get(ParseCoreReceiveXml.MSG_SIZE) + "&"
				+ ParseCoreReceiveXml.REQUEST_TYPE + "="
				+ map.get(ParseCoreReceiveXml.REQUEST_TYPE) + "&"
				+ ParseCoreReceiveXml.APP_USER + "=" 
	...
	public LocalResp verifyMessage(HashMap<String, String> map) {
		...
		if (map.get(ParseCoreReceiveXml.MSG_SIZE) == null
				|| map.get(ParseCoreReceiveXml.MSG_SIZE).trim().length() == 0) {
			i++;
