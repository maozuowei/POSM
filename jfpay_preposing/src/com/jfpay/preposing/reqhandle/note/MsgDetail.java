 ...
 * 
 */
public class MsgDetail extends ClientReqHandle implements IClientReqHandleService {
	...
	public CommunicationInfo getCommuInfo(HashMap<String, String> map) {
		...
		String parameter = ParseCoreReceiveXml.MOBILE_NO + "="
				...
				+ ParseCoreReceiveXml.MSG_ID + "="
				+ map.get(ParseCoreReceiveXml.MSG_ID);
	...
	public LocalResp verifyMessage(HashMap<String, String> map) {
		...
		if (map.get(ParseCoreReceiveXml.MSG_ID) == null
				|| map.get(ParseCoreReceiveXml.MSG_ID).trim().length() == 0) {
			i++;
			sb.append(ParseCoreReceiveXml.MSG_ID + ",");
		}
		if (i > 0) {
