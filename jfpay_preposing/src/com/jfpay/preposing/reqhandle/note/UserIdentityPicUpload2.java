 ...
 * 
 */
public class UserIdentityPicUpload2 extends ClientReqHandle implements IClientReqHandleService {
	...
	public CommunicationInfo getCommuInfo(HashMap<String, String> map) {
		...
		try {
			parameter = ParseCoreReceiveXml.APP_USER + "=" + map.get(ParseCoreReceiveXml.APP_USER) + "&" + 
					...
					ParseCoreReceiveXml.CUSTOMER_ID + "=" + map.get(ParseCoreReceiveXml.CUSTOMER_ID) + "&" + 
					ParseCoreReceiveXml.IMG + "=" + URLEncoder.encode(map.get(ParseCoreReceiveXml.IMG), "UTF-8") + "&" + 
					ParseCoreReceiveXml.IMG_APPLY_TYPE + "=" + map.get(ParseCoreReceiveXml.IMG_APPLY_TYPE) + "&" + 
					ParseCoreReceiveXml.IMG_SIGN + "=" + map.get(ParseCoreReceiveXml.IMG_SIGN);
	...
	public LocalResp verifyMessage(HashMap<String, String> map) {
		...
		if (map.get(ParseCoreReceiveXml.IMG) == null || map.get(ParseCoreReceiveXml.IMG).trim().length() == 0) {
			i++;
			sb.append(ParseCoreReceiveXml.IMG + ",");
		}
		if (map.get(ParseCoreReceiveXml.IMG_APPLY_TYPE) == null || map.get(ParseCoreReceiveXml.IMG_APPLY_TYPE).trim().length() == 0) {
			i++;
			sb.append(ParseCoreReceiveXml.IMG_APPLY_TYPE + ",");
		}
		if (map.get(ParseCoreReceiveXml.IMG_SIGN) == null || map.get(ParseCoreReceiveXml.IMG_SIGN).trim().length() == 0) {
			i++;
			sb.append(ParseCoreReceiveXml.IMG_SIGN + ",");
		}
		if (i > 0) {
