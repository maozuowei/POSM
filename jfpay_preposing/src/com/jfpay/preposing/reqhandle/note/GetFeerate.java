...
public class GetFeerate extends ClientReqHandle implements IClientReqHandleService{
	...
	public CommunicationInfo getCommuInfo(HashMap<String, String> map) {
		...
		parameter = ParseCoreReceiveXml.APP_USER + "=" + map.get(ParseCoreReceiveXml.APP_USER)
				+"&"+ParseCoreReceiveXml.VERSION_STATUS+"="+map.get(ParseCoreReceiveXml.VERSION_STATUS)
				+"&"+ParseCoreReceiveXml.TRADEDELAY+"="+map.get(ParseCoreReceiveXml.TRADEDELAY)
				+"&"+ParseCoreReceiveXml.MONEY+"="+map.get(ParseCoreReceiveXml.MONEY);
	...
	public LocalResp verifyMessage(HashMap<String, String> map) {
		...
		if (map.get(ParseCoreReceiveXml.VERSION_STATUS)==null||map.get(ParseCoreReceiveXml.VERSION_STATUS).trim().length()==0) {
			i++;
			sb.append(ParseCoreReceiveXml.VERSION_STATUS+",");
		}
		if (map.get(ParseCoreReceiveXml.TRADEDELAY)==null||map.get(ParseCoreReceiveXml.TRADEDELAY).trim().length()==0) {
			i++;
			sb.append(ParseCoreReceiveXml.TRADEDELAY+",");
		}
		if (map.get(ParseCoreReceiveXml.MONEY)==null||map.get(ParseCoreReceiveXml.MONEY).trim().length()==0) {
			i++;
			sb.append(ParseCoreReceiveXml.MONEY+",");
		}
		if (i > 0) {
