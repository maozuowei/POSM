...
public class GetAmountFee extends ClientReqHandle implements
	...
	public LocalResp verifyMessage(HashMap<String, String> map) {
		...
		if (map.get(ParseCoreReceiveXml.PRODUCT_ID) == null
				|| map.get(ParseCoreReceiveXml.PRODUCT_ID).trim().length() == 0) {
			i++;
			sb.append(ParseCoreReceiveXml.PRODUCT_ID + ",");
		}
		if (map.get(ParseCoreReceiveXml.AMOUNT) == null
				|| map.get(ParseCoreReceiveXml.AMOUNT).trim().length() == 0) {
			i++;
			sb.append(ParseCoreReceiveXml.AMOUNT + ",");
		}
		if (map.get(ParseCoreReceiveXml.PAY_TOOL) == null
				|| map.get(ParseCoreReceiveXml.PAY_TOOL).trim().length() == 0) {
			i++;
