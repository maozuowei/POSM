 ...
 *
 */
public class VerifyMobileMac extends ClientReqHandle implements IClientReqHandleService{

	@Override
	public CommunicationInfo getCommuInfo(HashMap<String, String> reqMap) {
	...
	public LocalResp verifyMessage(HashMap<String, String> map) {
		...
		if (map.get(ParseCoreReceiveXml.SMS_TYPE) == null
				|| map.get(ParseCoreReceiveXml.SMS_TYPE).trim().length() == 0) {
			i++;
			sb.append(ParseCoreReceiveXml.SMS_TYPE + ",");
		}
		if (map.get(ParseCoreReceiveXml.MOBILE_MAC) == null
				|| map.get(ParseCoreReceiveXml.MOBILE_MAC).trim().length() == 0) {
			i++;
