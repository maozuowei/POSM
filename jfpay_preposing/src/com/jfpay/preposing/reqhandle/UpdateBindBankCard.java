...
public class UpdateBindBankCard extends ClientReqHandle implements IClientReqHandleService {
	...
	public LocalResp verifyMessage(HashMap<String, String> map) {
		...
		if (map.get(ParseCoreReceiveXml.CARD_INDEX) == null
				|| map.get(ParseCoreReceiveXml.CARD_INDEX).trim().length() == 0) {
			i++;
		...
		if (map.get(ParseCoreReceiveXml.BANK_PROVINCE) == null
				|| map.get(ParseCoreReceiveXml.BANK_PROVINCE).trim().length() == 0) {
			i++;
			sb.append(ParseCoreReceiveXml.BANK_PROVINCE + ",");
		}
		if (map.get(ParseCoreReceiveXml.BANK_CITY) == null
				|| map.get(ParseCoreReceiveXml.BANK_CITY).trim().length() == 0) {
			i++;
			sb.append(ParseCoreReceiveXml.BANK_CITY + ",");
		}
		if (map.get(ParseCoreReceiveXml.BANK_NAME) == null
				|| map.get(ParseCoreReceiveXml.BANK_NAME).trim().length() == 0) {
			i++;
