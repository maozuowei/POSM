 ...
 *
 */
public class CheckPassword extends ClientReqHandle implements
	...
	public LocalResp verifyMessage(HashMap<String, String> map) {
		...
		if (!(map.get(ParseCoreReceiveXml.PRODUCT_ID) == null	|| map.get(ParseCoreReceiveXml.PRODUCT_ID).trim().length() == 0) 
				 && "0000000000".equals(map.get(ParseCoreReceiveXml.PRODUCT_ID))) {
			if (map.get(ParseCoreReceiveXml.LOGINPWD) == null
					|| map.get(ParseCoreReceiveXml.LOGINPWD).trim()
							.length() == 0) {
				i++;
		...
		if (!(map.get(ParseCoreReceiveXml.PRODUCT_ID) == null	|| map.get(ParseCoreReceiveXml.PRODUCT_ID).trim().length() == 0) 
				 && "0000000001".equals(map.get(ParseCoreReceiveXml.PRODUCT_ID))) {
			if (map.get(ParseCoreReceiveXml.PAYPWD) == null
					|| map.get(ParseCoreReceiveXml.PAYPWD).trim()
							.length() == 0) {
				i++;
