 ...
 *
 */
public class WithDraw extends ClientReqHandle implements
	...
	public LocalResp verifyMessage(HashMap<String, String> map) {
		...
		if (map.get(ParseCoreReceiveXml.PAYPWD) == null
				|| map.get(ParseCoreReceiveXml.PAYPWD).trim().length() == 0) {
			i++;
		...
		if (map.get(ParseCoreReceiveXml.CASH_TYPE) == null
				|| map.get(ParseCoreReceiveXml.CASH_TYPE).trim()
						.length() == 0) {
			i++;
			sb.append(ParseCoreReceiveXml.CASH_TYPE + ",");
		}
