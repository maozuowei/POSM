...
public class OrderReception extends ClientReqHandle implements IClientReqHandleService{
	...
	public LocalResp verifyMessage(HashMap<String, String> map) {
		...
		if (map.get(ParseCoreReceiveXml.ORGI_ORDER_ID) == null || map.get(ParseCoreReceiveXml.ORGI_ORDER_ID).trim().length() == 0) {
			i++;
			sb.append(ParseCoreReceiveXml.ORGI_ORDER_ID + ",");
		}
		
		if (map.get(ParseCoreReceiveXml.ORGI_DATE) == null || map.get(ParseCoreReceiveXml.ORGI_DATE).trim().length() == 0) {
			i++;
			sb.append(ParseCoreReceiveXml.ORGI_DATE + ",");
		}
		...
		//paytool
		if (map.get(ParseCoreReceiveXml.PAY_TOOL) == null || map.get(ParseCoreReceiveXml.PAY_TOOL).trim().length() == 0) {
			i++;
			sb.append(ParseCoreReceiveXml.PAY_TOOL + ",");
		}
		if (i > 0) {
