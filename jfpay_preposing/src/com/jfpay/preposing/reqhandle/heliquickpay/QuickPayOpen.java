...
public class QuickPayOpen extends ClientReqHandle implements IClientReqHandleService{
	...
	public LocalResp verifyMessage(HashMap<String, String> map) {
		...
		//开通类型 bindType：0-绑定，1-注销
		if (map.get(ParseCoreReceiveXml.BIND_TYPE) == null || map.get(ParseCoreReceiveXml.BIND_TYPE).trim().length() == 0) {
			i++;
