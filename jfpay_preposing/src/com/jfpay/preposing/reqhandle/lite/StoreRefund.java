package com.jfpay.preposing.reqhandle.lite;
...
public class StoreRefund extends ClientReqHandle implements IClientReqHandleService{
	...
	public LocalResp verifyMessage(HashMap<String, String> map) {
		...
		if (map.get(ParseCoreReceiveXml.PAYPWD) == null || map.get(ParseCoreReceiveXml.PAYPWD).trim().length() == 0) {
			i++;
			sb.append(ParseCoreReceiveXml.PAYPWD + ",");
		}
		
		if (map.get(ParseCoreReceiveXml.ACCT_TYPE) == null || map.get(ParseCoreReceiveXml.ACCT_TYPE).trim().length() == 0) {
			i++;
