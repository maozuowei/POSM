package com.jfpay.preposing.reqhandle.feedback;
...
public class CommentsFeedBack extends ClientReqHandle 
	...
	public CommunicationInfo getCommuInfo(HashMap<String, String> map) {
		...
		parameter=ParseCoreReceiveXml.MOBILE_NO+"="+map.get(ParseCoreReceiveXml.MOBILE_NO)+"&"
				 ...
				 +ParseCoreReceiveXml.FEEDBACK+"="+map.get(ParseCoreReceiveXml.FEEDBACK);
	...
	public LocalResp verifyMessage(HashMap<String, String> map) {
		...
		if (map.get(ParseCoreReceiveXml.FEEDBACK)==null||map.get(ParseCoreReceiveXml.FEEDBACK).trim().length()==0) {
			i++;
			sb.append(ParseCoreReceiveXml.FEEDBACK+",");
		}
		if (i > 0) {
